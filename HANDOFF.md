...
## Goal
A personal/internal dashboard that tracks AI events across **Brisbane and Ipswich** (Gold Coast was explicitly dropped). A Python scraper pulls events from Luma weekly into Supabase; a Next.js dashboard reads them, lets the user's friend mark events interested/attending/skip, and flags events that are NEW in the latest scrape. Built with the superpowers workflow (brainstorm → spec → plans → subagent-driven execution).
...
- **NEW clears each run** — an event is NEW iff `first_seen_run === max(scrape_runs.id)`. The writer stamps `first_seen_run` only on dedup_keys not already in the DB, so the previous run's badges self-clear when a new run gets a new id.
...
- **Weekly cron** — `0 2 * * 1` (Mondays 02:00 UTC). Was fortnightly (`0 2 1,15 * *`) until 2026-08-19; the ~14-day gap exceeded Supabase free-tier's ~7-day inactivity window, so the project auto-paused between runs and the scrape failed on DNS. Weekly writes double as the keep-alive.
...
- `.github/workflows/scrape.yml` — weekly scrape cron + manual dispatch
